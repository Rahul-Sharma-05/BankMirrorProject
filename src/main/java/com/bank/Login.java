		...
		if (bt.equalsIgnoreCase("AdminLogin")) {
			...
			try {
				...
				PreparedStatement pstm = con
						.prepareStatement("SELECT AdminName, password, status FROM Admin WHERE AdminId=?");
				pstm.setString(1, id);
...
//				rd.include(request, response);
		...
		if (bt.equalsIgnoreCase("CustomerLogin")) {
			...
			try {
				...
				PreparedStatement pstm = con
						.prepareStatement("SELECT Name,Password, IsAccount FROM Customer WHERE CustomerId=?");
				pstm.setString(1, id);
				ResultSet rs = pstm.executeQuery();
